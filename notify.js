...
let GOBOT_QQ = ''; // 如果 GOBOT_URL 设置 /send_private_msg 则需要填入 user_id=个人QQ 相反如果是 /send_group_msg 则需要填入 group_id=QQ群
...
let TG_PROXY_HOST = ''; // 例如：127.0.0.1
let TG_PROXY_PORT = ''; // 例如：1080
            ...
            $.post(options, (err, resp, data) => {
                try {
                    if (err) {
                        ...
                        data = JSON.parse(data);
                        if (data.code === 200) {
                            console.log(`pushplus 发送${PUSH_PLUS_USER ? '一对多' : '一对一'}通知消息完成。\n`);
                        } else if (data.code === 600) {
                            options['url'] = 'http://pushplus.hxtrip.com/send';
                            $.post(options, (e, r, d) => {
                                try {
                                    if (e) {
                                        console.log(`pushplus(hxtrip) 发送${PUSH_PLUS_USER ? '一对多' : '一对一'}通知消息失败！！\n`);
                                        console.log(e);
                                    } else {
                                        data = JSON.parse(d);
                                        if (data.code === 200) {
                                            console.log(`pushplus(hxtrip) 发送${PUSH_PLUS_USER ? '一对多' : '一对一'}通知消息完成。\n`);
                                        } else {
                                            console.log(`pushplus(hxtrip) 发送${PUSH_PLUS_USER ? '一对多' : '一对一'}通知消息失败：${data.msg}\n`);
                                        }
                                    }
                                } catch (error) {
                                    $.logErr(error, r);
                            ...
                            console.log(`pushplus 发送${PUSH_PLUS_USER ? '一对多' : '一对一'} 通知消息失败：${data.msg}\n`);
                        }
                    }
                } catch (e) {
                    $.logErr(e, resp);
